...
### 📅 每周 consolidation（总结提炼）

每天积累新词汇久了，知识库容易碎片化。因此每周一凌晨 4:00 多执行一轮审视：

| 步骤 | 说明 |
|------|------|
| **1. 全景分析** | 审视所有数据文件，识别可合并的重复条目、可提炼为规则的模式 |
| **2. Claude Code 提炼** | 调 Claude Code CLI 带着全部文件做总结重写，去碎片化 |
| **3. 评估采纳** | 保留有音无字词、文化专词等必要细节，只合并/精简冗余 |
| **4. GitHub 提交** | 更新后的知识库自动推送到 GitHub |

> **保留原则**：有音无字词、文化专词、粗口记录、区域变体—这些细节知识必须保留，不允许过度抽象。
> 
> **合并原则**：同义词合并、冗余例句替换、可泛化的模式提炼为语法规则放入 grammar.yaml。
