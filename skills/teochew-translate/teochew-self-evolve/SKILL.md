...
| `pages/negatives.md` | 否定词详解 | 🟢 全部 19 条已提取完毕 ✅ |
...
| `pages/overseas.md` | 海外潮汕话/借词 | 🟢 已启用 — 2026-06-25 首次提取，含闽南/马来/英语借词，主来源之一 |
...
| 無孬 (bho5(7)mo2) | 不坏 | ✅ 已提取（2026-06-25，批号5）|
...
所有 19 条已提取完毕，negatives.md 不再作为候选来源。参见 `references/negatives-md-extraction-log.md`。

**⚠️ 后续来源规划（2026-06-25）**：negatives.md 已全部提取完毕。以下 learn-teochew 页面可作为新来源：
...
#### ⚠️ 实战：overseas.md 页面提取（2026-06-25 新增）

与 negatives.md 的散文格式不同，**overseas.md 的结构是条目列表（heading-based）**，每条词汇为一个三级标题：

```
### *IPA* • *Peng'im* • 汉字 • "English meaning"
```

每条条目后续有一段或多段正文（含 IPA 和 Peng'im 例句），格式与 negatives.md 的 blockquote 类似。

**提取方法**：用 regex 匹配 `###` 开头的 heading 行即可快速获取所有条目：

```python
import json, base64, re

with open('/tmp/overseas.json') as f:
    data = json.load(f)
content = base64.b64decode(data['content']).decode('utf-8')
...
for line in content.split('\n'):
    ...
    if m:
        ...
        pengim = m.group(2).strip()
        chars = m.group(3).strip()
        entries.append((ipa, pengim, chars))
```

**字段说明**：overseas.md 的 heading 包含四个用 `•` 分隔的字段：
1. `*IPA*` — 方括音标（第一个 `*...*` 组）
2. `*Peng'im*` 或 `汉字` — 第二个元素是 Peng'im 或汉字
3. 剩余部分是汉字或英语释义

**主要内容类别**：
- 海外潮汕特有词（如 番畔 huang1 boin5、唐山 deng5 suan1）
- 闽南语借词（如 查某 za1 bhou2、淡薄 dam7 boh8）
- 马来语借词（如 鐳 lui1=钱、baru=刚/新）
- 英语借词（名词为主，如 selat=新加坡）

⚠️ 注意 overseas.md 中的很多词是东南亚社区特有的，标注其来源和地区限制。
